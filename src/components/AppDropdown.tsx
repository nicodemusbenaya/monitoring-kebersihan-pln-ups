...
export function AppDateField({
  ...
  const parsed = useMemo(() => {
    if (!value) { const d = new Date(); return { y: d.getFullYear(), m: d.getMonth() + 1, d: d.getDate() }; }
  ...
  useEffect(() => { setViewYear(parsed.y); setViewMonth(parsed.m); }, [parsed.y, parsed.m]);
  ...
  const monthLabel = useMemo(() => {
    return new Intl.DateTimeFormat("id-ID", { month: "long", year: "numeric" }).format(new Date(viewYear, viewMonth - 1, 1));
  }, [viewYear, viewMonth]);
  ...
  return (
    <div ref={ref} className="relative min-w-[200px]">
      ...
      >
        <span className={`text-[13px] font-bold truncate ${value ? "text-[#17313d]" : "text-[#94a3b8]"}`}>{display}</span>
        <span className={`shrink-0 w-7 h-7 rounded-lg bg-white border flex items-center justify-center ${open ? "border-[#ffd100] text-[#0076a8]" : "border-[#e2e8f0] text-[#64748b]"}`}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4"/><path d="M8 2v4"/><path d="M3 10h18"/></svg>
        </span>
      </button>
      ...
      {open && (
        <div className="absolute z-50 mt-2 w-full min-w-[320px] bg-white border border-[#d8e3ea] rounded-2xl shadow-xl p-3">
          {/* Header September 2026 */}
          <div className="flex items-center justify-between pb-3 border-b border-[#eef2f6] mb-2">
            <button type="button" onClick={goPrev} className="w-8 h-8 rounded-lg bg-[#eef6fb] hover:bg-[#e0eef7] text-[#5f7d8e] flex items-center justify-center">
              <ChevronDown className="w-4 h-4 rotate-90" />
            </button>
            <span className="text-sm font-black text-[#17313d] capitalize">{monthLabel}</span>
            <button type="button" onClick={goNext} className="w-8 h-8 rounded-lg bg-[#eef6fb] hover:bg-[#e0eef7] text-[#5f7d8e] flex items-center justify-center">
              <ChevronDown className="w-4 h-4 -rotate-90" />
            </button>
          </div>
          ...
          <div className="grid grid-cols-7 gap-1 mb-1">
            {WEEKDAYS.map((w) => (
              <span key={w} className="h-7 flex items-center justify-center text-[11px] font-bold text-[#8aa0ad]">{w}</span>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              const day = i + 1;
              const isSelected = value === `${viewYear}-${String(viewMonth).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
              const isToday = !value && day === new Date().getDate() && viewMonth === new Date().getMonth() + 1 && viewYear === new Date().getFullYear();
              return (
                <button
                  key={day}
                  type="button"
                  ...
                  className={`h-9 rounded-xl text-[13px] font-bold flex items-center justify-center transition-all ${
                    isSelected
                      ? "bg-[#ffd100] text-[#17313d] shadow-md"
                      : isToday
                      ? "bg-[#eef6fb] text-[#0076a8] border border-[#0076a8]/20"
                      : "text-[#334a5a] hover:bg-[#f1f5f9]"
                  }`}
                >
                  {day}
                </button>
