...
export default function AdminDashboardPage() {
  ...
  const fetchEvaluationsData = async (start?: string, end?: string, room?: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/evaluations?startDate=${s}&endDate=${e}&roomId=${rId}`).then((r) => r.json());
      if (res.ok) setEvaluationsData(res.data);
    } catch (err) {
  ...
  const loadAllData = async () => {
    setLoading(true);
    try {
      const [userRes, dashRes, perfRes, evalRes, roomsRes, usersRes, previewRes] = await Promise.all([
        ...
        fetch(`/api/admin/evaluations?startDate=${evalStartDate}&endDate=${evalEndDate}&roomId=${evalRoomFilter}`).then((r) => r.json()),
    ...
    <div className="flex h-screen w-screen overflow-hidden bg-[#edf2f6] font-sans text-[#17313d] select-none">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        ...
        <main className="p-8 max-w-[1500px] w-full mx-auto space-y-6">
          ...
          {/* ══════════════════════ TAB 3: KEPUASAN PENGGUNA (Screenshots 7 & 8) ══════════════════════ */}
          {activeNav === "kepuasan" && (
            <div className="space-y-6">
              {/* Header Title */}
              <div>
          ...
          {/* ══════════════════════ TAB 5: EKSPOR EXCEL (Screenshots 9 & 10) ══════════════════════ */}
          {activeNav === "export" && (
            <div className="space-y-6">
              {/* Header Title */}
              <div>
              ...
              {exportActiveTab === "room" && (
                <div className="space-y-6">
                  ...
                  <section className="bg-white border border-[#d8e3ea] rounded-2xl p-6 shadow-sm space-y-4">
                    ...
                    {/* PLN Styled Excel Table Sheet Preview (Screenshot 10) */}
                              ...
                              {(exportPreviewData?.activities || [
                                ...
                                <tr key={act.id} className="hover:bg-[#f8fafc]">
                                  ...
                                  {(exportPreviewData?.days || [{ dateKey: "2026-09-01" }, { dateKey: "2026-09-02" }, { dateKey: "2026-09-03" }]).map((day: any, dIdx: number) => {
                                    // Day 1 has inspection by Sulaiman (Pagi Pos)
                                    const isDay1 = dIdx === 0;
